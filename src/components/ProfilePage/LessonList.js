...
const LessonList = (props) => {
  const [lessons, updateLessons] = useState([]);
  const [lessonCount, updateCount] = useState();
  ...
  return (
    <React.Fragment>
      <Container style={props.style}>
        <Header as="h2" style={styles.header} inverted color="red">
          Lessons
        </Header>
        <LessonModalForm user={props.user} onAdd={onLessonAddition} />
        <Card.Group style={styles.card}>
          {lessonCount == 0 && (
            <EmptyListFallback user={props.user} url={props.url} />
          )}
          {lessonCount > 0 &&
            lessons.map((lesson, idx) => (
              <LessonItem
