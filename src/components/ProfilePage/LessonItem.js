...
const LessonItem = (props) => {
  ...
  const handleDelete = (e) => {
    ...
    if (
      props.user === props.originUserName ||
      props.originUserName == undefined
    ) {
      axios
        .delete(`${baseURI}/users/${props.user}/lesson/${props.id}`)
        .then((r) => {
    ...
    return (
      <Card fluid style={{ padding: "10px" }}>
        ...
        <p style={{ fontFamily: "Consolas" }}>
          <strong>
            Seems like you're new here since you don't have any personal lessons
            yet. But don't worry, we got you covered. Here are some favorites
            from{" "}
            <a target="_blank" href="https://github.com/ankingcodes">
              @ankingcodes's
            </a>{" "}
            lessons.
          </strong>
